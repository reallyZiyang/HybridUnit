...
public static class BattleCollisionMath
{
    ...
    public static Rect RotatedRectAabb(Vector2 center, Vector2 size, float rotationDeg)
    {
        Vector2 halfSize = new Vector2(Mathf.Max(0f, size.x) * 0.5f, Mathf.Max(0f, size.y) * 0.5f);
        Quaternion rotation = Quaternion.Euler(0f, 0f, rotationDeg);
        Vector2 c0 = center + (Vector2)(rotation * new Vector3(-halfSize.x, -halfSize.y, 0f));
        Vector2 c1 = center + (Vector2)(rotation * new Vector3(-halfSize.x, halfSize.y, 0f));
        Vector2 c2 = center + (Vector2)(rotation * new Vector3(halfSize.x, halfSize.y, 0f));
        Vector2 c3 = center + (Vector2)(rotation * new Vector3(halfSize.x, -halfSize.y, 0f));
        return CreateAabb(c0, c1, c2, c3);
    }

    public static Rect SectorAabb(Vector2 center, Vector2 forward, float radius, float angleDeg)
    {
        return CircleAabb(center, radius);
    }
    ...
    public static Rect Expand(Rect rect, float amount)
    {
        ...
        return Rect.MinMaxRect(rect.xMin - safeAmount, rect.yMin - safeAmount, rect.xMax + safeAmount, rect.yMax + safeAmount);
    }
    ...
    private static Rect CreateAabb(params Vector2[] points)
    {
